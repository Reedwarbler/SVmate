###Bug fix:1. 11/06/2017, clipped part shoudl be reverse complemented first before align.
#             Otherwise, cannot check the orientation consistence between the two parts.
#          2. add the "-k" option for bwa mem.
...
def is_qualified_clipped(cigar, cutoff_len):
    ...
    if (signal[0] == "S" or signal[0] == "H") and lenth[0] >= cutoff_len:  # left-clip
        ...
        if signal[0] == "H":
            b_hardclip = True
    if (signal[len(signal) - 1] == "S" or signal[len(signal) - 1] == "H") and lenth[
                len(signal) - 1] >= cutoff_len:  # right-clip
...
def parse_alignment_gnrt_disc_clip(sf_sam, m_insert_size, sf_clip):
    ...
    with open(sf_sam) as fin_sam, open(sf_clip, "w") as fout_clip:
        ...
        for line in fin_sam:
            ...
            if cigar == "*":  # unmapped are not interested
                continue
            ...
            #check whether reverse complementary, if yes, then reverse complement the clipped part
            s_clip_seq=clipped_seq
            s_clip_qseq=clipped_qseq
            if flag & 16 != 0:
                s_clip_seq= gnrt_reverse_complementary(clipped_seq)
                s_clip_qseq=clipped_qseq[::-1]

            ##parse out the clipped part and save into sf_clip
            fout_clip.write("@" + qname + "_" + str(cnt) + "_" + str(bleft_clip) + "\n")
            fout_clip.write(s_clip_seq + "\n")
            fout_clip.write("+\n")
            fout_clip.write(s_clip_qseq + "\n")
...
def check_clip_consistent(mpart1, mpart2, start, end, b_expect_ori, dist_clip_pos):
    ...
    for qname in mpart1:
        ...
        if (b_ori == b_expect_ori) and (
            b_within_dist1 or b_within_dist2):  # same direction, and "clip_pos" is within specific distance from the "breakpoints"
            m_clip[qname] = b_rc1
...
def run_parse_region(regions):
    ...
    cmd = "{0} mem -T {1} -k {2} {3} {4} > {5}".format(BWA_PATH, CLIP_CUTOFF, CLIP_CUTOFF, sref, sf_clip, sf_clip_align)
    run_cmd(cmd)
...
def parse_option():
    ...
    parser.add_option("-d", "--direction", dest="direction",
                      help="The expected direction for the split parts: True for same direction) ", metavar="FILE")
...
####
# source activate /home/cc439/new_virtualenv
# python cvt_isinfo.py /n/data1/hms/dbmi/park/simon_chu/projects/autism_case/UMB1024-pfc-1b12_1-final_meerkat2/UMB1024-pfc-1b12_1-final-all.isinfo is_info.txt
# python SVReadsRetrieve.py -t -r /home/sl279/BiO/Install/GATK-bundle/2.8/b37/human_g1k_v37_decoy.fasta -b /n/data1/hms/dbmi/park/DATA/Library_Prep_Comparison/bam/UMB1024-pfc-1b12_1-final-all.bam  -v in.bed -o test -n 2 -e 1000 -i is_info.txt
####
